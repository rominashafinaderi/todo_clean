...
class EditBloc extends Bloc<EditEvent, EditState> {
  ...
  EditBloc({required this.todoUseCase}) : super(EditInitial()) {
    on<EditEvent>(_onEditTodo);
  }

  Future<void> _onEditTodo(EditEvent event, Emitter<EditState> emit) async {
    emit(EditLoading());
    final result = await todoUseCase.updateTodosFromRepo(event.todo.id, event.todo.userId, event.todo.title, event.todo.completed);
